...
app.post('/ins', async (req, res) => {
  ...
  // Verificação de CPF
  const cpfExists = await Scheduling.findOne({ cpf: newUser.cpf });
  ...
  await Scheduling.create(newUser);
  return res.status(201).json(newUser);
});
app.put('/ins/:id', async (req, res) => {
  const { id } = req.params;
  const updateUser = req.body;
  await Scheduling.findByIdAndUpdate(id, updateUser);
...
app.put('/login/user/:id', async (req, res) => {
  ...
  const salt = await bcrypt.genSalt(12);
  const passwordHash = await bcrypt.hash(password, salt, (err, hash) => null);
